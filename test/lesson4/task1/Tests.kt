...
class Tests {
    ...
    @Tag("Impossible")
    fun russian() {
        assertEquals("пятьсот четыре тысячи восемьсот девяносто", russian(504890))
        assertEquals("девятьсот шестьдесят четыре тысячи четыреста восемьдесят шесть", russian(964486))
        assertEquals("шестьсот пять тысяч триста пятьдесят восемь", russian(605358))
        assertEquals("сто девятнадцать тысяч пятьсот восемь", russian(119508))
        assertEquals("восемьсот тридцать шесть тысяч триста", russian(836300))
